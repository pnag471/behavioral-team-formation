...
# ---------------------------------------------------------------------------
# Fixed mock signature returned by the analyzer for all tests
# ---------------------------------------------------------------------------
MOCK_SIG = {
    ...
    "accountability": "high",
    "help_seeking": "proactive",
    "confidence_score": 0.85,
}

# Six complete responses — one per scenario
ALL_RESPONSES = [
    {"scenario_id": "missed_deadline",        "option_id": "collaborative",   "response_text": ""},
    {"scenario_id": "workload_imbalance",     "option_id": "collaborative",   "response_text": ""},
    {"scenario_id": "communication_breakdown","option_id": "facilitative",    "response_text": ""},
    {"scenario_id": "leadership_vacuum",      "option_id": "facilitative",    "response_text": ""},
    {"scenario_id": "technical_disagreement", "option_id": "collaborative",   "response_text": ""},
    {"scenario_id": "ambiguous_requirements", "option_id": "facilitative",    "response_text": ""},
]


# ---------------------------------------------------------------------------
# Fixture: fresh in-memory DB + mocked analyzer for each test
# ---------------------------------------------------------------------------
@pytest.fixture
def client():
    # StaticPool shares a single connection — required for SQLite in-memory
    # so that tables created by Base.metadata.create_all are visible to all sessions.
    ...
    db_module.init_db = lambda: None  # suppress file-based table creation in lifespan

    with patch("app.behavioral_analysis.analyzer.analyze", return_value=MOCK_SIG):
...
def test_analyze_creates_student(client):
    ...
    try:
        ...
        sig = db.query(BehavioralSignatureDB).filter(BehavioralSignatureDB.student_id == student_id).first()
        assert sig is not None
        assert sig.signature["planning_style"] == "planner"
    finally:
        db.close()
...
def test_analyze_updates_existing_student(client):
    ...
    # First submission — creates the student
    ...
    # Second submission with the same student_id
    ...
    try:
        ...
        assert row.work_rhythm_signature["planning_style"] == "planner"
    finally:
        db.close()
...
def test_save_progress(client):
    ...
    try:
        ...
        assert session.status == "incomplete", "Status should remain incomplete after save-progress"
...
def test_analyze_completes_existing_session(client):
    ...
    db = db_module.SessionLocal()
    try:
        # Must be exactly one AssessmentDB row for this student
